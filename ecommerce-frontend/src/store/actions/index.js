...
export const increaseCartQuantity = (data, toast, currentQuantity, setCurrentQuantity) => (dispatch, getState) => {
    ...
    if (isQuantityExist) {
        ...
        //we change the local state and localstorage
...
export const stripePaymentConfirmation = (sendData, setErrorMesssage, setLoading, toast) => async (dispatch, getState) => {
    try {
...
export const addNewProductFromDashboard = (sendData, toast, reset, setLoader, setOpen) => async(dispatch, getState) => {
    try {
        setLoader(true);
...
export const addNewDashboardSeller = (sendData, toast, reset, setOpen, setLoader) => async (dispatch) => {
    try {
      setLoader(true);
